...
struct CompositeOp
{
    inline __hostdev__ void operator()(float* outImage, int i, int w, int h, int label, float alpha) const
    {
        int semanticColorMap[3*31] = {
            ...
            124, 252, 0};  // 30: Lawn Green

        int instanceColorMap[3*1024] = {
            0, 0, 0,
            255, 255, 0,
            28, 230, 255,
            255, 52, 255,
...
//             color.r = d_SCM[3*sem_label];
//             color.g = d_SCM[3*sem_label+1];
//             color.b = d_SCM[3*sem_label+2];
//         }
//         else { // instance label
//             color.r = d_ICM[3*inst_label];
//             color.b = d_ICM[3*inst_label+1];
//             color.g = d_ICM[3*inst_label+2];
//         }
// #else
        if (inst_label == 0) { // no instance label, so go off of semantics
            color.r = semanticColorMap[3*sem_label];
            color.g = semanticColorMap[3*sem_label+1];
            color.b = semanticColorMap[3*sem_label+2];
        }
        else { // instance label
            color.r = instanceColorMap[3*inst_label];
            color.g = instanceColorMap[3*inst_label+1];
            color.b = instanceColorMap[3*inst_label+2];
        }
// #endif

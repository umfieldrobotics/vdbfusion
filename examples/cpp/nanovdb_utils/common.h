...
struct CompositeOp
{
    ...
    {
        std::array<RGB, 31> semanticColorMap = {{
        ...
        std::array<RGB, 1024> instanceColorMap = {{
            {0, 0, 0},
            {255, 255, 0},
            {28, 230, 255},
            {255, 52, 255},
        ...
        // printf("upper: %d lower: %d\n", inst_label, sem_label);

        RGB color;
        if (inst_label == 0) { // no instance label, so go off of semantics
            color = semanticColorMap[sem_label];
        }
        else { // instance label
            color = instanceColorMap[inst_label];
        }
